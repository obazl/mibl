...
char *fn =  "normalize-stanza-fld-flags";
...
void test_template(void) {
    utstring_renew(sexp);
    utstring_printf(sexp, "(%s %s :common)", fn,
...
void test_flags(void) {
    ...
    utstring_renew(sexp);
    utstring_printf(sexp, "(%s %s :common)", fn,
                    "'(flags (-nopervasives :standard -nostdlib))");
    actual = s7_eval_c_string(s7, utstring_body(sexp));
    expected = s7_eval_c_string(s7,
        "'(:opts (:standard) (:flags -nopervasives -nostdlib))");
    TEST_ASSERT_TRUE(s7_is_equal(s7, actual, expected));
    ...
    utstring_renew(sexp);
    utstring_printf(sexp, "(%s %s :common)", fn,
                    "'(flags (-nopervasives -nostdlib :standard))");
    actual = s7_eval_c_string(s7, utstring_body(sexp));
    expected = s7_eval_c_string(s7,
        "'(:opts (:standard) (:flags -nopervasives -nostdlib))");
    TEST_ASSERT_TRUE(s7_is_equal(s7, actual, expected));
    ...
    utstring_renew(sexp);
    utstring_printf(sexp, "(%s %s :common)", fn,
                    "'(flags (-nopervasives -nostdlib))");
    actual = s7_eval_c_string(s7, utstring_body(sexp));
    expected = s7_eval_c_string(s7,
        "'(:opts (:flags -nopervasives -nostdlib))");
    TEST_ASSERT_TRUE(s7_is_equal(s7, actual, expected));
}
...
void test_no_sublist(void) {
    ...
    utstring_renew(sexp); // add whitespace
    utstring_printf(sexp, "(%s %s :common)", fn,
                    "'(flags\t:standard -nopervasives -nostdlib)");
    actual = s7_eval_c_string(s7, utstring_body(sexp));
    expected = s7_eval_c_string(s7,
...
void test_options(void) {
    utstring_renew(sexp);
    utstring_printf(sexp, "(%s %s :common)", fn,
                    "'(flags (:standard -foo a -bar b))");
    actual = s7_eval_c_string(s7, utstring_body(sexp));
    expected = s7_eval_c_string(s7,
         "'(:opts (:standard) (:options (-foo . a) (-bar . b)))");
    TEST_ASSERT_TRUE(s7_is_equal(s7, actual, expected));
    ...
    actual = s7_eval_c_string(s7, utstring_body(sexp));
    expected = s7_eval_c_string(s7,
         "'(:opts (:options (-foo . a) (-bar . b)))");
    TEST_ASSERT_TRUE(s7_is_equal(s7, actual, expected));
}

void test_string_opts(void) {
    utstring_renew(sexp);
    utstring_printf(sexp, "(%s %s :common)", fn,
                    "'(flags (:standard -foo \"a\" -bar \"b\"))");
    actual = s7_eval_c_string(s7, utstring_body(sexp));
...
void test_int_opts(void) {
    utstring_renew(sexp);
    utstring_printf(sexp, "(%s %s :common)", fn,
                    "'(flags (:standard -foo 1 -bar 2))");
    actual = s7_eval_c_string(s7, utstring_body(sexp));
    expected = s7_eval_c_string(s7,
         "'(:opts (:standard) (:options (-foo . 1) (-bar . 2)))");
    TEST_ASSERT_TRUE(s7_is_equal(s7, actual, expected));
    ...
    actual = s7_eval_c_string(s7, utstring_body(sexp));
    expected = s7_eval_c_string(s7,
         "'(:opts (:options (-foo . 1) (-bar . 2)))");
    TEST_ASSERT_TRUE(s7_is_equal(s7, actual, expected));

}

void test_float_opts(void) {
    utstring_renew(sexp);
    utstring_printf(sexp, "(%s %s :common)", fn,
                    "'(flags (:standard -foo 1.0 -bar 2.3))");
    actual = s7_eval_c_string(s7, utstring_body(sexp));
    expected = s7_eval_c_string(s7,
         "'(:opts (:standard) (:options (-foo . 1.0) (-bar . 2.3)))");
    TEST_ASSERT_TRUE(s7_is_equal(s7, actual, expected));

    // no :standard
    utstring_renew(sexp);
    utstring_printf(sexp, "(%s %s :common)", fn,
                    "'(flags -foo 1.0 -bar 2.3)");
    actual = s7_eval_c_string(s7, utstring_body(sexp));
    expected = s7_eval_c_string(s7,
         "'(:opts (:options (-foo . 1.0) (-bar . 2.3)))");
    TEST_ASSERT_TRUE(s7_is_equal(s7, actual, expected));
}

void test_opens(void) {
    utstring_renew(sexp);
    utstring_printf(sexp, "(%s %s :common)", fn,
                    "'(flags (:standard -open amod -open bmod))");
    actual = s7_eval_c_string(s7, utstring_body(sexp));
    expected = s7_eval_c_string(s7,
         "'(:opts (:standard) (:opens amod bmod))");
    TEST_ASSERT_TRUE(s7_is_equal(s7, actual, expected));
    ...
    actual = s7_eval_c_string(s7, utstring_body(sexp));
    expected = s7_eval_c_string(s7,
         "'(:opts (:opens amod bmod))");
    TEST_ASSERT_TRUE(s7_is_equal(s7, actual, expected));
}

void test_flags_opts(void) {
    utstring_renew(sexp);
    utstring_printf(sexp, "(%s %s :common)", fn,
       "'(flags (:standard -foo a -bar b -nopervasives -nostdlib))");
    actual = s7_eval_c_string(s7, utstring_body(sexp));
    expected = s7_eval_c_string(s7,
     "'(:opts (:standard) (:options (-foo . a) (-bar . b)) (:flags -nopervasives -nostdlib))");
    TEST_ASSERT_TRUE(s7_is_equal(s7, actual, expected));

    utstring_renew(sexp); // permutation
    utstring_printf(sexp, "(%s %s :common)", fn,
       "'(flags (:standard  -nopervasives -foo a -bar b -nostdlib))");
    actual = s7_eval_c_string(s7, utstring_body(sexp));
    expected = s7_eval_c_string(s7,
     "'(:opts (:standard) (:options (-foo . a) (-bar . b)) (:flags -nopervasives -nostdlib))");
    TEST_ASSERT_TRUE(s7_is_equal(s7, actual, expected));
    ...
    utstring_renew(sexp); // permutation
    utstring_printf(sexp, "(%s %s :common)", fn,
       "'(flags (:standard  -nopervasives -nostdlib -foo a -bar b))");
    actual = s7_eval_c_string(s7, utstring_body(sexp));
    char *s = s7_object_to_c_string(s7, actual);
    log_debug("result: %s", s);
    free(s);
    ...
    actual = s7_eval_c_string(s7, utstring_body(sexp));
    expected = s7_eval_c_string(s7,
     "'(:opts (:options (-foo . a) (-bar . b)) (:flags -nopervasives -nostdlib))");
    TEST_ASSERT_TRUE(s7_is_equal(s7, actual, expected));
...
int main(int argc, char **argv)
{
    ...
    RUN_TEST(test_no_sublist);
    RUN_TEST(test_options);
    RUN_TEST(test_string_opts);
    RUN_TEST(test_int_opts);
    RUN_TEST(test_float_opts);
    RUN_TEST(test_opens);
    RUN_TEST(test_flags_opts);
