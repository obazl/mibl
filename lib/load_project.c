...
bool _is_ws_root(FTSENT *ftsentry)
{
    ...
    if (!rc) {
        ...
        if (!rc) {
#if defined(PROFILE_fastbuild)
            if (mibl_debug) log_trace("true");
#endif
            return true;
        }
    }
#if defined(PROFILE_fastbuild)
    if (mibl_debug) log_trace("false");
#endif
    return false;
}
...
LOCAL s7_pointer make_pkg_key(s7_scheme *s7, char *path)
{
    if (path[0] == '.' && path[1] == '\0') {
...
/*
  ...
  OCaml source file.
 */
LOCAL void _handle_dir(s7_scheme *s7,
                       s7_pointer pkg_tbl, FTS* tree, FTSENT *ftsentry)
{
#if defined(PROFILE_fastbuild)
/* #if defined(TRACING) */
...
/* #if defined(TRACING) */
    ...
    /* drop leading "./" */
    s7_pointer pkg_key = make_pkg_key(s7, ftsentry->fts_path);
...
LOCAL void _update_pkg_files(s7_scheme *s7,
                             s7_pointer pkg_tbl,
...
LOCAL void _update_cc_src_file(s7_scheme *s7,
                               s7_pointer pkg_tbl,
                               FTSENT *ftsentry, char *ext)
{
#if defined(PROFILE_fastbuild)
...
LOCAL void _update_cc_hdr_file(s7_scheme *s7,
                               s7_pointer pkg_tbl,
                               FTSENT *ftsentry, char *ext)
{
#if defined(PROFILE_fastbuild)
...
LOCAL void _update_pkg_script_files(s7_scheme *s7,
                                    s7_pointer pkg_tbl,
                                    FTSENT *ftsentry, char *ext)
{
    TRACE_ENTRY;
...
LOCAL void _update_pkg_modules(s7_scheme *s7,
                               s7_pointer pkg_tbl,
...
LOCAL void _update_pkg_sigs(s7_scheme *s7,
                            s7_pointer pkg_tbl,
                            char *pkg_name, char *mname,
                            char *fname, int ftype)
{
...
LOCAL void _update_pkg_structs(s7_scheme *s7,
                               s7_pointer pkg_tbl,
                               char *pkg_name, char *mname,
                               char *fname, int ftype)
{
...
LOCAL void _update_pkg_mll_files(s7_scheme *s7,
                                 s7_pointer pkg_tbl,
                                 char *pkg_name, char *mname,
                                 char *fname, int ftype)
{
...
LOCAL void _update_mll(s7_scheme *s7,
                       s7_pointer pkg_tbl,
                       FTSENT *ftsentry, char *ext)
{
    TRACE_ENTRY;
    ...
    if (rc) {
        ...
        _update_pkg_structs(s7, pkg_tbl, pkg_name, mname,
                            ml_name, // ftsentry->fts_name,
                            TAG_ML_DYN);
    } else {
        /* _update_pkg_modules(s7, pkg_tbl, pkg_name, mname, */
    ...
    _update_pkg_mll_files(s7, pkg_tbl, pkg_name, mname,
...
LOCAL void _update_mly(s7_scheme *s7,
                       s7_pointer pkg_tbl,
                       FTSENT *ftsentry, char *ext)
{
    TRACE_ENTRY;
   ...
   _update_pkg_modules(s7, pkg_tbl, pkg_name, mname,
    ...
    _update_pkg_mly_files(s7, pkg_tbl, pkg_name, mname,
...
LOCAL void _update_pkg_cppo_files(s7_scheme *s7,
                                  s7_pointer pkg_tbl,
                                  char *pkg_name, char *mname,
                                  char *fname, int ftype)
{
...
LOCAL void _update_cppo(s7_scheme *s7,
                        s7_pointer pkg_tbl,
                        FTSENT *ftsentry, char *ext)
{
    TRACE_ENTRY;
...
/*     } */
    ...
    /* update pkg fld :cppo */
    _update_pkg_cppo_files(s7, pkg_tbl, pkg_name, mname,
...
LOCAL void _update_mli(s7_scheme *s7,
                       s7_pointer pkg_tbl,
                       FTSENT *ftsentry, char *ext)
{
    TRACE_ENTRY;
    ...
    if (rc) {
        ...
        _update_pkg_sigs(s7, pkg_tbl, pkg_path, mname,
                         ftsentry->fts_name, TAG_MLI);
    } else {
        _update_pkg_modules(s7, pkg_tbl, pkg_path, mname,
                            ftsentry->fts_name, TAG_MLI);
    }
}

LOCAL void _update_ml(s7_scheme *s7,
                      s7_pointer pkg_tbl,
                      FTSENT *ftsentry, char *ext)
{
    TRACE_ENTRY;
...
#endif
        _update_pkg_structs(s7, pkg_tbl, pkg_path, mname,
                            ftsentry->fts_name, TAG_ML);
    } else {
#if defined(PROFILE_fastbuild)
        if (mibl_debug_traversal)
 ...
 */
LOCAL void _handle_ml_file(s7_scheme *s7,
                           s7_pointer pkg_tbl,
                           FTSENT *ftsentry, char *ext)
{
    TRACE_ENTRY;
    ...
    if ((strncmp(ext, ".ml", 3) == 0)
        ...
        if (ln > 7) {
            if (strncmp((ftsentry->fts_name) + (ln-7), "cppo", 4) == 0) {
                _update_cppo(s7, pkg_tbl, ftsentry, ext);
            } else {
                _update_ml(s7, pkg_tbl, ftsentry, ext);
            }
        } else {
            _update_ml(s7, pkg_tbl, ftsentry, ext);
        }
        ...
        if (ln > 8) {
            if (strncmp((ftsentry->fts_name) + (ln-8), "cppo", 4) == 0) {
                ...
                _update_mli(s7, pkg_tbl, ftsentry, ext);
            }
        } else {
            _update_mli(s7, pkg_tbl, ftsentry, ext);
        }
    ...
    /*  */
        ...
        _update_pkg_files(s7, pkg_tbl, ftsentry, ext);
    }
        ...
        && (strlen(ext) == 4)) {
        _update_mll(s7, pkg_tbl, ftsentry, ext);
    }
    else if ((strncmp(ext, ".mly", 4) == 0)
        && (strlen(ext) == 4)) {
        _update_mly(s7, pkg_tbl, ftsentry, ext);
    }
...
LOCAL void _handle_file(s7_scheme *s7,
                        s7_pointer pkg_tbl, FTSENT *ftsentry)
{
    file_ct++;
    ...
    if (strncmp(ftsentry->fts_name, "dune-project", 12)
        == 0) {
        _handle_dune_project_file(s7, pkg_tbl, ftsentry);
        return; // break;
    }
    if ((strncmp(ftsentry->fts_name, "dune", 4) == 0)
        /* don't read dune.foo */
        && (strlen(ftsentry->fts_name) == 4)) {
        _handle_dune_file(s7, pkg_tbl, ftsentry);
        /* break; */
        return; // continue;
    }
    ...
    if (ext) {
        if ((strncmp(ext, ".toml", 5) == 0)
                 && (strlen(ext) == 5)) {
            _handle_toml_file(s7, pkg_tbl, ftsentry);
        }
        if ((strncmp(ext, ".json", 5) == 0)
                 && (strlen(ext) == 5)) {
            _handle_json_file(s7, pkg_tbl, ftsentry);
        }
        else if ((strncmp(ext, ".cm", 3) == 0)) {
            log_debug("skipping .cm? file : %s", ftsentry->fts_name);
        }
        else if (strncmp(ext, ".ml", 3) == 0) {
            /* handle_ml_file will analyze the full extension */
            _handle_ml_file(s7, pkg_tbl, ftsentry, ext);
        }
        else if ((strncmp(ext, ".md", 3) == 0)
                 && (strlen(ext) == 3)) {
            ...
            /*_handle_script_file(s7, pkg_tbl, ftsentry, ext);*/
        }
            ...
            /*_handle_script_file(s7, pkg_tbl, ftsentry, ext);*/
                 ...
                 && (strlen(ext) == 5)) {
            _handle_opam_file(s7, pkg_tbl, ftsentry);
        }
        else if (fnmatch("*.opam.template",
                         ftsentry->fts_name, 0) == 0) {
            _handle_opam_template_file(/*s7,*/ pkg_tbl, ftsentry);
        }
        else if (strncmp(ext, ".ocamlformat", 12) == 0) {
            _handle_ocamlformat_file(/* s7, */ pkg_tbl, ftsentry);
        }
        else if ((strncmp(ext, ".c", 2) == 0)
                 && (strlen(ext) == 2)) {
            _update_cc_src_file(s7, pkg_tbl, ftsentry, ext);
            /* _handle_cc_file(s7, pkg_tbl, ftsentry, ext); */
        }
        else if ((strncmp(ext, ".h", 2) == 0)
                 && (strlen(ext) == 2)) {
            _update_cc_hdr_file(s7, pkg_tbl, ftsentry, ext);
            /* _handle_cc_file(s7, pkg_tbl, ftsentry, ext); */
        }
...
LOCAL void _handle_generic_file(s7_scheme *s7,
                                s7_pointer pkg_tbl,
                                FTSENT *ftsentry, char *ext)
{
    TRACE_ENTRY;
...
LOCAL void _handle_toml_file(s7_scheme *s7,
                             s7_pointer pkg_tbl,
                             FTSENT *ftsentry)
{
    TRACE_ENTRY;
...
LOCAL void _handle_json_file(s7_scheme *s7,
                             s7_pointer pkg_tbl,
                             FTSENT *ftsentry)
{
    TRACE_ENTRY;

    s7_pointer json_obj = json_read_file(s7, ftsentry->fts_path);

    /* json_obj = s7_make_string(s7, "JSON TEST"); */

    /* TRACE_S7_DUMP("json_obj", json_obj); */
    /* fflush(NULL); */
    ...
    if (pkg_alist == s7_f(s7)) {
        ...
        if (json_assoc == s7_f(s7)) {
#if defined(PROFILE_fastbuild)
            if (mibl_debug_traversal)
                log_debug("initializing (:json ...)");
#endif

            // create new json entry in *mibl-ws*
            // e.g. for dune.json:  (:json (dune <#json-table ...>))
            // we know the extension is '.json'
            char *stem = strdup(ftsentry->fts_name);
            stem[strlen(stem) - 5] = '\0';
            log_debug("STEM: %s", stem);
            s7_pointer json_file_sym = s7_make_symbol(s7, stem);
            free(stem);

            s7_pointer json_list =
                s7_list(s7, 1,
                        s7_cons(s7,
                                json_kw,
                                s7_list(s7, 1,
            ...
            s7_pointer json_file_assoc = // json_assoc? s7_cons
                s7_list(s7, 1, s7_list(s7, 2,
                                       json_file_sym,
                                       json_obj
                                       //s7_make_string(s7, "JSON TEST")
                                       ));
#if defined(PROFILE_fastbuild)
            if (mibl_debug_traversal) {
...
LOCAL void _handle_dune_file(s7_scheme *s7,
                             s7_pointer pkg_tbl,
                             FTSENT *ftsentry)
{
    TRACE_ENTRY;
    ...
    s7_pointer dune_read = s7_name_to_value(s7, "dune:read");
    s7_pointer inport = s7_open_input_file(s7,
                                           ftsentry->fts_path,
                                           "r");
    s7_pointer stanzas = s7_apply_function(s7, dune_read, s7_list(s7, 1, inport));
...
LOCAL void _update_opam_files(s7_scheme *s7,
                              s7_pointer pkg_tbl,
                              FTSENT *ftsentry) //, char *ext)
{
    TRACE_ENTRY;
...
LOCAL void _handle_opam_file(s7_scheme *s7,
                             s7_pointer pkg_tbl,
                             FTSENT *ftsentry)
{
    TRACE_ENTRY;
    ...
    _update_opam_files(s7, pkg_tbl, ftsentry);
}

LOCAL void _handle_opam_template_file(// s7_scheme *s7,
                                      s7_pointer pkg_tbl,
                                      FTSENT *ftsentry)
{
    (void)pkg_tbl;
    ...
    /* _update_ml(s7, ftsentry, ext); */
}

LOCAL void _handle_ocamlformat_file(// s7_scheme *s7,
                                    s7_pointer pkg_tbl,
                                    FTSENT *ftsentry)
{
    (void)pkg_tbl;
...
LOCAL __attribute__((unused)) void _handle_script_file(s7_scheme *s7,
                                                       s7_pointer pkg_tbl,
                                                       FTSENT *ftsentry, char *ext)
{
    /* if (mibl_debug_traversal) */
    ...
    _update_pkg_script_files(s7, pkg_tbl, ftsentry, ext);
}

LOCAL void _handle_cc_file(s7_scheme *s7,
                           s7_pointer pkg_tbl,
                           FTSENT *ftsentry, char *ext)
{
    /* if (mibl_debug_traversal) { */
...
LOCAL void _handle_symlink(s7_scheme *s7,
                           s7_pointer pkg_tbl,
                           FTS *tree, FTSENT *ftsentry)
{
    if (strncmp(ftsentry->fts_name, "bazel-", 6) == 0) {
        /* skip Bazel dirs, e.g. bazel-bin */
        if (verbose)
            log_info(RED "Excluding" CRESET " Bazel symlink: %s", ftsentry->fts_name);
        fts_set(tree, ftsentry, FTS_SKIP);
        return;
    }
    ...
    _handle_file(s7, pkg_tbl, ftsentry);
    return;
...
LOCAL __attribute__((unused)) const char *_get_path_dir(s7_scheme *s7,
                                                        s7_pointer arg)
{
    TRACE_ENTRY;
...
EXPORT s7_pointer g_load_project(s7_scheme *s7,  s7_pointer args)
{
    ...
    if ( s7_is_null(s7, args)
        ...
        pathdir = ".";
        /* s7_pointer _pkg_tbl = */
        load_project(s7, rootdir, pathdir);
        /* if (mibl_trace) { */
#if defined(PROFILE_fastbuild)
        if (mibl_debug)
...
/* #endif */
...
#if defined(PROFILE_fastbuild)
    if (mibl_debug) log_trace("srch ptr: %s", ptr);
#endif
    char **p;
    p = NULL;
    ...
    if  (p != NULL) {
#if defined(PROFILE_fastbuild)
        if (mibl_debug_traversal) { // & (verbosity > 2)) {
        ...
        while ( (p=(char**)utarray_next(mibl_config.include_dirs, p))) {
            ...
            if (mibl_debug) {
                log_debug("inclusion test pfx: '%s', path: '%s'",
                      *p, ftsentry->fts_path);
                int include = strncmp(*p, ftsentry->fts_path, strlen(*p));
                log_debug("result: %d", include);
                log_debug("including? %d", include > 0);
            }
#endif
...
EXPORT s7_pointer load_project(s7_scheme *s7,
                               const char *home_sfx,
            ...
            switch (ftsentry->fts_info)
                ...
                case FTS_D : // dir visited in pre-order
                    ...
                    if (traverse_dir(tree, ftsentry)) {
                        ...
                        dir_ct++;
                        _handle_dir(s7, pkg_tbl, tree, ftsentry);
                        /* printf("pkg tbl: %s\n", TO_STR(pkg_tbl)); */
                    }
                    break;
                case FTS_DP:
                ...
                case FTS_F : // regular file
                    ...
                    break;
                case FTS_SL: // symlink
                    _handle_symlink(s7, pkg_tbl, tree, ftsentry);
                    break;
                case FTS_SLNONE:
