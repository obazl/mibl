...
(define (normalize-run-action pkg action-alist targets deps)
  ...
  (let* ((stanza-type :run-cmd)
         ...
         (_ (if *mibl-debug-s7* (format #t "action-alist: ~A\n" action-alist)))
         (run-alist (assoc-val 'run action-alist))
         (_ (if *mibl-debug-s7* (format #t "run-alist: ~A\n" run-alist)))

         (pkg-path (assoc-val :pkg-path pkg))
         ;; (_ (if *mibl-debug-s7* (format #t "pkg-path: ~A\n" pkg-path)))

         (tool (run-action->toolname pkg-path run-alist))
         (_ (if *mibl-debug-s7* (format #t "TOOL: ~A\n" tool)))

         ;; (tool-tag (normalize-tool-tag (cadadr action)))
         ;; (_ (if *mibl-debug-s7* (format #t "TOOL-TAG: ~A\n" tool-tag)))

         ;; (run-list (cadr action)) ;; (run <tool> <arg1> ...)
         ;; ;; (_ (if *mibl-debug-s7* (format #t "run-list: ~A\n" run-list)))

         ;; (target (if-let ((target (assoc 'target rule-alist)))
         ;;                 (cadr target) #f))
         ;; ;; (_ (if *mibl-debug-s7* (format #t "target: ~A\n" target)))
         ;; (targets (if-let ((targets (assoc 'targets rule-alist)))
         ;;                  (cadr targets)
         ;;                  #f))
         ;; ;; (_ (if *mibl-debug-s7* (format #t "targets: ~A\n" targets)))
         ...
         ;; ;; (_ (if *mibl-debug-s7* (format #t "outfile: ~A\n" outfile)))

         (args (cdr run-alist))
          ;;(run-action->args pkg-path action run-list))
         (_ (if *mibl-debug-s7* (format #t "CMD ARGS: ~A\n" args)))
         ...
         ;;  )
         (_ (if *mibl-debug-s7* (format #t "CMD DEPS: ~A\n" deps)))
