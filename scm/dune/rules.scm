...
;; better: just use dsl:action->mibl directly?
(define (-normalize-rule-action ws pkg tools rule-alist targets deps)
  ...
  (let* ((mibl-action (rule-action:normalize ws pkg rule-alist tools targets deps))
         ...
         ;; (_ (if (or *mibl-debug-rule-stanzas* *mibl-debug-all*) (format #t "rule-tag: ~A\n" rule-tag)))
         ;; (_ (if (or *mibl-debug-rule-stanzas* *mibl-debug-all*) (format #t "~A: ~A\n" (green "TARGETS") targets)))
         ...
         (r-alist (if alias (cons (list ':alias alias) r-alist)
                      r-alist))
         ;; (r-alist (append r-alist `((::cmd ,@new-action))))
...
(define x-rule-explicit->mibl
  (let ((+documentation+ "INTERNAL. Updates pkg arg, returns normalized stanza. stanza: raw dune stanza (input); nstanza: miblized (output)"))
    (lambda (ws pkg stanza)
      (mibl-trace-entry "x-rule-explicit->mibl" "")
      (mibl-trace "ws" ws)
      ...
      (let* ((pkg-path (assoc-val :pkg-path pkg))
             ...
             (tools `(::tools :DUMMY1)) ;;  placeholder, to be replaced by set-cdr!
                          ...
                          (case (cdr dep)
                            ...
                            (else
                             ...
                             (let* ((lbl-tag (car dep))
                                    ...
                                    (tgt-tag (caadr lbl))
                                    (_ (if (or *mibl-debug-rule-stanzas* *mibl-debug-all*) (format #t "~A: ~A~%" (red "tgt-tag") tgt-tag)))
                                    (tgt (case tgt-tag
                                           ((:tgt)
                                            (assoc-val :tgt lbl))
                                           ((:tgts)
        ...
        (let* ((mibl-rule `((:rulex ,(if (truthy? deps) deps (values))
                                    ,targets
                                    ;; all dune rule stanzas have an action, so they have a tool
...
(define x-rule-inferred->mibl
  ...
  (let ((+documentation+ "INTERNAL. Updates pkg arg, returns normalized stanza. stanza: raw dune stanza (input); nstanza: miblized (output)"))
    (lambda (ws pkg stanza)
      (mibl-trace-entry "x-rule-inferred->mibl" "")
      (mibl-trace "ws" ws)
      (mibl-trace "pkg" (assoc-val :pkg-path pkg))
      (mibl-trace "stanza" stanza)
      ;; for other stanza types we can normalize fields in isolation. For
      ...
      (let* ((pkg-path (assoc-val :pkg-path pkg))
             ...
             (action (find-if (lambda (fld)
                                ;; (format #t "~A: ~A~%" (red "RULE FLD") fld)
                                (member (car fld) dune-dsl-cmds))
                              rule-alist))
             (mibl-trace-let "rule action" action)
             (action (car (dsl:action->mibl (list action))))
             (mibl-trace-let "rule action (mibled)" action)
             ...
             ;; all dune rule stanzas have an action, so they have a tool
             (tools `(::tools :DUMMY3)) ;; a placeholder, to be replaced by set-cdr!
             (mibl-trace-let "tmp ::tools" tools)
             )
  ...
  (let ((+documentation+ "INTERNAL. Updates pkg arg, returns normalized stanza. stanza: raw dune stanza (input); nstanza: miblized (output)"))
    (lambda (ws pkg stanza)
      ...
      (let* ((pkg-path (assoc-val :pkg-path pkg))
             ...
             ;; all dune rule stanzas have an action, so they have a tool
             (tools `(::tools :DUMMY5)) ;; a placeholder, to be replaced by set-cdr!
             (mibl-trace-let "tmp ::tools" tools)
             )
...
(define -dune-rule-inferred->mibl
  (let ((+documentation+ "INTERNAL. Updates pkg arg, returns normalized stanza. stanza: raw dune stanza (input); nstanza: miblized (output)"))
    (lambda (ws pkg stanza)
      ...
      (let* ((pkg-path (assoc-val :pkg-path pkg))
             (rule-alist (cdr stanza))
             (_ (if (or *mibl-debug-rule-stanzas* *mibl-debug-all*) (format #t "rule-alist: ~A\n" rule-alist)))
             ;; (_ (if (or *mibl-debug-rule-stanzas* *mibl-debug-all*) (format #t "target: ~A\n" (assoc 'target rule-alist))))
             ;; (_ (if (or *mibl-debug-rule-stanzas* *mibl-debug-all*) (format #t "Targets: ~A\n" (assoc-val 'targets rule-alist))))
             ...
             ;; all dune rule stanzas have an action (even if they have no 'action fld),
             ;; so they have a tool
             (tools `(::tools :DUMMY6)) ;; a placeholder, to be replaced by set-cdr!
             (mibl-trace-let "tmp ::tools" tools)
             )
        ...
        (if (or *mibl-debug-rule-stanzas* *mibl-debug-all*)
            (format #t "~A: ~A~%" (ugreen "DISPATCHING  on action (infer)") rule-alist))

        (let* ((mibl-rule
                ;; this routine is for rules w/o 'action fld
               ...
               (mibl-rule (prune-mibl-rule mibl-rule)))
          (if (or *mibl-debug-rule-stanzas* *mibl-debug-all*)
              (format #t "~A: ~A~%" (green "INFERRED mibl-rule") mibl-rule))
    ...
    (lambda (ws pkg stanza)
      ...
      (if (assoc 'action (cdr stanza))
          (list
           ...
           (x-rule-explicit->mibl ws pkg stanza))
          (list
           (-dune-rule-inferred->mibl ws pkg stanza)
           (x-rule-inferred->mibl ws pkg stanza))))))
