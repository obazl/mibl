...
(define (X-alias-args->miblark pkg stanza)
  ...
  (if-let ((args (assoc-in '(:actions :cmd :args) (cdr stanza))))
          ...
          (let* ((stanza-alist (cdr stanza))
                 (_ (if *mibl-debug-all* (format #t "~A: ~A~%" (uwhite "stanza-alist") stanza-alist)))
                 (alias (cadr (assoc :alias stanza-alist)))
                 (_ (if *mibl-debug-all* (format #t "~A: ~A~%" (uwhite "alias") alias)))
                 (args (cdr args))
                 (_ (if *mibl-debug-all* (format #t "~A: ~A~%" (uwhite "alias args") args)))
                 (cmd-ct (length (assoc-in* '(:actions :cmd) stanza-alist)))
                 (_ (if *mibl-debug-all* (format #t "~A: ~A~%" (uwhite "cmd ct") cmd-ct)))
                 (deps (if-let ((deps (assoc :deps stanza-alist)))
                               (cdr deps) '()))
                 (_ (if *mibl-debug-all* (format #t "~A: ~A~%" (uwhite "deps") deps)))
                 (outputs (if-let ((outputs (assoc ::outputs stanza-alist)))
                               (cdr outputs) '()))
                 (_ (if *mibl-debug-all* (format #t "~A: ~A~%" (uwhite "outputs") outputs)))

                 ;; (tool (format #f "~A" (keyword->symbol (car args))))
                 ;; (_ (if *mibl-debug-all* (format #t "~A: ~A~%" (uwhite "tool") tool)))
            ...
            ;;   ;;                     ;; add ::tools to (:deps ...)
            ;;   ;;                     (let ((_ (if *mibl-debug-all* (format #t "~A: ~A~%" (ured "deps") deps)))
            ;;   ;;                           (deps-list (cdr deps))
