...
(define (find-module-in-pkg module pkg)
  ...
  (let* ((pkg-modules (if-let ((files (assoc-val :modules pkg)))
                                files '()))
         (_ (if *mibl-debugging* (format #t "~A: ~A~%" (yellow "pkg-modules") pkg-modules)))
         ;; (m-name (filename->module-name arg))
         ;; (_ (if *mibl-debugging* (format #t "~A: ~A~%" (yellow "m-name") m-name)))
         (entry
          (find-if (lambda (mod)
    ...
    (if entry
        ...
        (let* ((structs-static (if-let ((ss (assoc-in '(:structures :static) pkg))) (cdr ss) '()))
               ...
               (_ (if *mibl-debugging* (format #t "~A: ~A~%" (yellow "pkg-structs") pkg-structs)))
               ;; (m-name (filename->module-name arg))
               ;; (_ (if *mibl-debugging* (format #t "~A: ~A~%" (yellow "m-name") m-name)))
               (entry
...
(define (-find-m-file-in-pkg-structs arg pkg)
  ...
  (let* ((statics (if-let ((files (assoc-in '(:structures :static) pkg)))
                          (cdr files) '()))
         ;; (_ (if *mibl-debugging* (format #t "~A: ~A~%" (yellow "statics") statics)))
         (dynamics (if-let ((files (assoc-in '(:structures :dynamic) pkg)))
                           (cdr files) '()))
         ;; (_ (if *mibl-debugging* (format #t "~A: ~A~%" (yellow "dynamics") dynamics)))
         (structs (concatenate statics dynamics))
         ;; (_ (if *mibl-debugging* (format #t "~A: ~A~%" (yellow "structs") structs)))
         (file
          (find-if (lambda (struct)
...
(define (-find-m-file-in-pkg-sigs arg pkg)
  ...
  (let* ((pkg-sigs (if-let ((files (assoc-val :signatures pkg)))
                              files '()))
         ;; (_ (if *mibl-debugging* (format #t "~A: ~A~%" (yellow "pkg-sigs") pkg-sigs)))
         (file
          (find-if (lambda (sig)
...
(define (is-module-in-pkg m pkg)
  ...
  (let* ((pkg-mods (if-let ((files (assoc-val :modules pkg)))
                             (map car files) '()))
         ;; (_ (if *mibl-debugging* (format #t "~A: ~A~%" (yellow "pkg-mods") pkg-mods)))
         (pkg-structs (if-let ((structs (assoc-val :structures pkg)))
                              (let* (;; (_ (if *mibl-debugging* (format #t "~A: ~A~%" (uyellow "pkg-structs") structs)))
                                     (statics (if-let ((statics (assoc-val :static structs)))
                                                      statics '()))
                                     ;; (_ (if *mibl-debugging* (format #t "~A: ~A~%" (cyan "struct statics") statics)))
                                     (dynamics (if-let ((dynamics (assoc-val :dynamic structs)))
                                                       dynamics '()))
                                     ;; (_ (if *mibl-debugging* (format #t "~A: ~A~%" (cyan "struct dynamics") dynamics)))
         ...
         (_ (if *mibl-debugging* (format #t "~A: ~A~%" (yellow "all pkg modules") modules)))
      ...
      (let* ((pkg-files (if-let ((files (assoc-val :files pkg)))
             ...
             (pkg-files (append
                         ...
                         (if-let ((dynamics (assoc-val :dynamic pkg-files)))
                                 dynamics '())))
             (_ (if *mibl-debugging* (format #t "~A: ~A~%" (cyan "pkg-files") pkg-files)))
             (file
              (find-if (lambda (f)
                         (if *mibl-debugging*
...
(define (-codept-pkg-file-deps pkg-path)
  ...
  (let* ((fpath (format #f "~A/*" pkg-path)) ;; (cdr struct)))
         (cmd (format #f "codept -k -expand-deps -sexp -I ~A ~A/*"
                      pkg-path pkg-path))
         (_ (if *mibl-debugging* (format #t "~A: ~A~%" (green "cmd") cmd))))
...
(define (-ocamldep-pkg-static-file-deps! pkg)
  ...
  (let* ((pkg-path (car (assoc-val :pkg-path pkg)))
         ...
         (pkg-sigs (if-let ((sigs (assoc-val :signatures pkg)))
                              (let* (;; (_ (if *mibl-debugging* (format #t "~A: ~A~%" (uyellow "pkg-sigs") sigs)))
                                     (statics (if-let ((statics (assoc-val :static sigs)))
                                                      statics '()))
                                     ;; (_ (if *mibl-debugging* (format #t "~A: ~A~%" (cyan "sig statics") statics)))
                                     (dynamics (if-let ((dynamics (assoc-val :dynamic sigs)))
                                                       dynamics '()))
                                     ;; (_ (if *mibl-debugging* (format #t "~A: ~A~%" (cyan "sig dynamics") dynamics)))
                                     ) ;; gets both :static & :dynamic
                                (map car (concatenate statics dynamics)))
                           '()))
         (_ (if *mibl-debugging* (format #t "~A: ~A~%" (yellow "pkg-sigs") pkg-sigs)))
                  ...
                  (let ((segs (string-split dep #\:)))
                    ...
                    (if (null? (cdr segs))
                        ...
                        (let* ((fpath (car segs))
                               ...
                               (_ (if *mibl-debugging* (format #t "~A for ~A: ~A~%" (ugreen "unfiltered mdeps") mname mdeps)))
                               ;; do not include file module in deps list
                               (mdeps (remove mname mdeps))
                               (_ (if *mibl-debugging* (format #t "~A: ~A~%" (ugreen "unfiltered mdeps excluding self") mdeps)))
                               ...
                               (_ (if *mibl-debugging* (format #t "~A: ~A~%" (ugreen "filtered mdeps, excluding pkg-externals") mdeps)))
                               )
                          (if *mibl-debugging*
                              (begin
...
(define (-ocamldep-pkg-dynamic-file-deps! pkg)
  ...
  (let* ((ws-path (car (assoc-val :ws-path pkg)))
         ...
         (pkg-static-mll (if-let ((mll-static
                                 ...
                                 (cdr mll-static) '()))
         (_ (if *mibl-debugging* (format #t "~A: ~A~%" (blue "pkg-static-mll") pkg-static-mll)))
         ;; prolly won't have dynamic .mll files, but just in case
         (pkg-dynamic-mll (if-let ((mll-dyn (assoc-in '(:lex :dynamic) pkg)))
                                  (cdr mll-dyn) '()))
         (_ (if *mibl-debugging* (format #t "~A: ~A~%" (blue "pkg-dynamic-mll") pkg-dynamic-mll)))

         (pkg-mll (concatenate pkg-static-mll pkg-dynamic-mll))
         (_ (if *mibl-debugging* (format #t "~A: ~A~%" (blue "pkg-mll") pkg-mll)))

         (pkg-static-mly (if-let ((mly-static
                                   (assoc-in '(:yacc :static) pkg)))
                                 mly-static '()))
         (_ (if *mibl-debugging* (format #t "~A: ~A~%" (blue "pkg-static-mly") pkg-static-mly)))
         (pkg-dynamic-mly (if-let ((mly-dyn
                                    (assoc-in '(:yacc :dynamic) pkg)))
                                  mly-dyn '()))
         (_ (if *mibl-debugging* (format #t "~A: ~A~%" (blue "pkg-dynamic-mly") pkg-dynamic-mly)))
         (pkg-mly (concatenate pkg-static-mly pkg-dynamic-mly))
         (_ (if *mibl-debugging* (format #t "~A: ~A~%" (blue "pkg-mly") pkg-mly)))
...
;;                        (pkg (cdr pkg-kv))
;;                        ;; (_ (if *mibl-debugging* (format #t "~A: ~A~%" (green "pkg") pkg)))
