  ...
  (let recur ((opts opts)
    ...
    (if (null? opts)
        (if (null? ostack)
            ...
            (values options (cons (car ostack) flags) (reverse orphans)))
        (let* ((rawopt (car opts))
               ;; (_ (format #t "rawopt: ~A\n" rawopt))
               (opt (cond
          ...
          (if (number? opt)
              (if (null? ostack)
                  ...
                  (recur (cdr opts) '()
                         (cons (cons (car ostack) rawopt) options)
                         flags orphans))
                      ...
                      (begin
                        ...
                        (if (or (equal? (car ostack) "-w")
                            ...
                            (begin
                              ...
                              (let ((prev (car ostack)))
                                (recur (cdr opts)
                                       (cons (symbol opt) (cdr ostack)) ;; new opt replaces prev on stack
                                       options
                                       (cons prev flags)
                                       orphans)))
