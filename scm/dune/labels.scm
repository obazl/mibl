...
(define (-fixup-std-dep-form ws pkg dep exports)
  ...
  (case (cdr dep)
     ...
     (let ((exp (hash-table-ref exports
       ...
       (if exp
           (let* ((pkg (assoc-val :pkg exp))
                  (_ (if *mibl-debugging* (format #t "~A: ~A~%" (ured "pkg") pkg)))
                  (tgt (assoc-val :tgt exp))
                  (_ (if *mibl-debugging* (format #t "~A: ~A~%" (ured "tgt") tgt))))
             (cons (car dep) exp))
    ...
    ;;  (let* ((exp (hash-table-ref exports
    ;;                              (car dep)))
    ;;         (_ (if *mibl-debugging* (format #t "~A: ~A~%" (yellow "exp") exp)))
...
(define (-fixup-dep-sym ws-id dep pkg-path exports)
  ...
  (if-let ((x (assoc-val dep ocaml-std-pkgs)))
          ...
          (let* ((key (string->keyword
                       (format #f "lib:~A" dep)))
                 (_ (if *mibl-debugging* (format #t "~A: ~A~%" (uwhite "trying") key)))
                 (resolved (hash-table-ref exports key)))
            (if resolved
                (let* ((pkg (assoc-val :pkg resolved))
                       (_ (if *mibl-debugging* (format #t "~A: ~A~%" (ured "pkg") pkg)))
                       (_ (if *mibl-debugging* (format #t "~A: ~A~%" (ured "pkg-path") pkg-path)))
                ...
                (let* ((key (string->keyword
                             (format #f "~A" dep)))
                       (_ (if *mibl-debugging* (format #t "~A: ~A~%" (uwhite "trying2") key)))
                       ;; (_ (if *mibl-debugging* (format #t "~A: ~A~%" (bgblue "exports") exports)))
                  ...
                  (if resolved
                      ...
                      (let* ((key (string->symbol
                                   (format #f "~A" dep)))
                             (_ (if *mibl-debugging* (format #t "~A: ~A~%" (uwhite "trying3") key)))
                             (resolved (hash-table-ref exports key)))
                        (if resolved
                            (let* ((pkg (assoc-val :pkg resolved))
                                   (_ (if *mibl-debugging* (format #t "~A: ~A~%" (ured "pkg") pkg)))
            ...
            (case (car stanza)
               ...
               (let* (;; (modules (assoc-in '(:compile :manifest :modules) stanza-alist))
                     ...
                     (stanza-deps (assoc-in '(:deps :remote) stanza-alist))
                      ...
                      (ppxex (if-let ((ppxes (assoc-val :ppxes stanza-alist)))
                                     ppxes #f))
                      (_ (if *mibl-debugging* (format #t "~A: ~A~%" (ublue "ppx") ppx)))
                 ...
                 (if ppx
                     (begin
                       ...
                       (let* ((exports (car (assoc-val :exports ws)))
                              ...
                              (_ (if *mibl-debugging* (format #t "~A: ~A~%" (bgred "ppx-deps") ppx-deps)))
                              (fixppx
                               (map (lambda (dep)
                                      (if *mibl-debugging*
              ...
              ((:rule :diff :node :ocamlc)
               ...
               (let* ((targets (assoc-val :outputs stanza-alist))
                      (_ (if *mibl-debugging* (format #t "targets: ~A~%" targets)))
                      (deps (if-let ((deps (assoc :deps stanza-alist)))
                                    ;; (if (null? deps) '() (car deps))
                                    deps
                                    #f))
                      (_ (if *mibl-debugging* (format #t "deps: ~A~%" deps)))
                      ...
                      (_ (if *mibl-debugging* (format #t "action: ~A~%" action)))
                 ...
                 (if (assoc :actions stanza-alist)
                     ...
                     (begin
                       ...
                       ;;         (let* ((_ (if *mibl-debugging* (format #t "tool-label: ~A~%" tool-label)))
                       ;;                (pkg (car (assoc-val :pkg tool-label)))
                       ;;                (tgt (car (assoc-val :tgt tool-label)))
                       ;;                (label (format #f "//~A:~A" pkg tgt))
                       ;;                (_ (if *mibl-debugging* (format #t "tool-label: ~A\n" tool-label))))
              ...
              ((:archive :ns-archive :library :ns-library)
               ...
               (let* ((deps (if-let ((deps (assoc-in '(:deps :remote) stanza-alist)))
                                    deps #f))
                      (_ (if *mibl-debugging* (format #t "~A: ~A~%" (blue "deps") deps)))
                      (ppx (if-let ((ppx (assoc-val :ppx stanza-alist)))
                                   ppx #f))
                      (ppxex (if-let ((ppxes (assoc-val :ppxes stanza-alist)))
                                     ppxes #f))
    ...
    (lambda (ws-id)
      (let ((ws (assoc-val ws-id *mibl-project*)))
        ...
        (let* ((pkgs (car (assoc-val :pkgs ws)))
               ;; (_ (if *mibl-debugging* (format #t "PKGS: ~A\n" pkgs)))
               (exports (car (assoc-val :exports ws))))
