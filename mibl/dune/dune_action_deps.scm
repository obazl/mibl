...
(define (handle-filename-literal-arg ws dep paths)
  ...
  (let* (;; (_ (if *debugging* (format #t "dep: ~A\n" dep)))
         (pkg-path (car (assoc-val :pkg-path paths)))
         (_ (if *debugging* (format #t "pkg-path: ~A~%" pkg-path)))
         ...
         (_ (if *debugging* (format #t "dep-path: ~A~%" dep-path)))

         (canonical-path (->canonical-path dep-path))
         (_ (if *debugging* (format #t "canonical-path: ~A~%" canonical-path)))

         (kind (if (file-exists? canonical-path)
                   :static :dynamic))
         (_ (if *debugging* (format #t "~A: ~A~%" "kind" kind)))

         (_ (if *debugging* (format #t "(dirname canonical-path) ~A~%"
         ...
         (_ (if *debugging* (format #t "local? ~A~%" local?)))

         (path (if local?
                   dep
                   canonical-path))
                   ;; (string-append pkg-path "/" canonical-path)))
         (_ (if *debugging* (format #t "path: ~A~%" path)))

         ;; (expanded-path (if local? path canonical-path))
         (expanded-path canonical-path)
         (_ (if *debugging* (format #t "~A: ~A~%" (bgred "expanded-path") expanded-path)))
         ...
         (_ (if *debugging* (format #t "~A: ~A~%" (bgred "TGT") tgt)))
...
(define (handle-tagged-literal-dep ws deplist paths expanded-deps)
  ...
  (let* ((tag (car deplist))
         (_ (if *debugging* (format #t "~A: ~A~%" (yellow "tag is kw?") (keyword? tag))))
...
(define (handle-tagged-glob-dep ws tagged-pattern paths expanded-deps)
  ...
  (let* ((lbl (string->keyword (format #f "~A" (keyword->symbol (car tagged-pattern)))))
         ...
         (_ (if *debugging* (format #t "~A: ~A~%" (green "pattern") pattern)))
         ...
         (_ (if *debugging* (format #t "~A: ~A~%" (ublue "fg-name") fg-name)))
...
(define (handle-untagged-glob-dep ws paths _pattern)
  ...
  (let* ((pattern (cadr _pattern))
         (pkg-path (car (assoc-val :pkg-path paths)))
         (dep-path (format #f "~A/~A" pkg-path pattern))
         (_ (if *debugging* (format #t "~A: ~A~%" (uwhite "dep-path") dep-path)))

         (_ (if *debugging* (format #t "~A: ~A~%" (bgred "cwd") (pwd))))
         (canonical-path (->canonical-path dep-path))
         (_ (if *debugging* (format #t "~A: ~A~%" (uwhite "canonical-path") canonical-path)))
